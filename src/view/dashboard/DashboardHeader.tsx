    ...
    <View className="mt-10 bg-primary w-full p-4 rounded-xl">
      <View className="flex flex-row items-center justify-between">
        <Animated.View
          ...
          className="bg-white/20 h-6 w-32 rounded"
        />
      </View>
      ...
      <View className="mt-4 flex flex-row items-center gap-5">
        <Animated.View
          style={{ opacity: pulseAnim }}
          className="bg-white/20 w-10 h-10 rounded-full"
        />
        <View className="flex-1">
          <View className="flex flex-row items-center justify-between">
            <Animated.View
              style={{ opacity: pulseAnim }}
              className="bg-white/20 h-5 w-24 rounded"
            />
            <Animated.View
              style={{ opacity: pulseAnim }}
              className="bg-white/20 h-5 w-8 rounded"
            />
          </View>
          <Animated.View
            style={{ opacity: pulseAnim }}
            className="mt-2 bg-white/20 h-[.3rem] w-full rounded-full"
          />
        </View>
      </View>
        ...
        {[1, 2, 3].map((_, index) => (
          <View key={index} className="flex flex-row items-center gap-2">
            ...
            <View className="flex flex-row items-center gap-1">
              <Animated.View
                style={{ opacity: pulseAnim }}
                className="bg-white/20 h-5 w-6 rounded"
              />
              <Animated.View
                style={{ opacity: pulseAnim }}
                className="bg-white/20 h-4 w-12 rounded"
              />
            </View>
          </View>
        ))}
      ...
      <View className="mt-10 bg-primary w-full p-4 rounded-xl">
        ...
        <View className="flex flex-row items-center justify-between ">
          ...
          <View className="flex flex-row items-center gap-2">
            ...
            <View className="flex flex-row items-center gap-1">
              <Text className="text-white font-medium">
                {(stats?.total_sessions || 0) - (stats?.sessions_attended || 0)}
              </Text>
